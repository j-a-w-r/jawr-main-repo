	...
	private void initImageMapping(ImageResourcesHandler imgRsHandler) {
		...
		if (jawrConfig.getUseBundleMapping()
			...
			while (mapIterator.hasNext()) {
				...
				imgRsHandler.addMapping((String) entry.getKey(), entry
						.getValue().toString());
			}
	...
	private void addItemsFromDir(ImageResourcesHandler imgRsHandler,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Adding " + resources.size()
					+ " resources from path [" + dirName + "] to image bundle");
		}

		GeneratorRegistry generatorRegistry = imgRsHandler.getConfig()
	...
	private void writeContent(HttpServletResponse response, String fileName)
		...
		if (is == null) {
			...
			if (LOGGER.isInfoEnabled())
				LOGGER.info("Received a request for a non existing image resource: "
						+ resourceName);
			return;
		}
