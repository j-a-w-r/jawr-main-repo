...
public class ServletContextUtils {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(ServletContextUtils.class);
	...
	/**
	 * The getContextPath method name to retrieve the context path for servlet
	 * API 2.5 and above
	 */
	...
	public static String getContextPath(ServletContext servletContext) {
		...
		if (servletContext != null) {
			// If the servlet API version is greater or equals to 2.5, use the
			...
			} else { // Retrieve the context path from the init parameter or the
						// servlet context
				contextPath = servletContext
						.getInitParameter(CONTEXT_PATH_PARAM_NAME);
			}
	...
	private static String getContextPathForNewServletApi(
		...
		String contextPath = null;
		try {
			Method getServletContextPathMethod = servletContext.getClass()
					.getMethod(GET_CONTEXT_PATH_METHOD, new Class[] {});
			contextPath = (String) getServletContextPathMethod.invoke(
					servletContext, (Object[]) null);
