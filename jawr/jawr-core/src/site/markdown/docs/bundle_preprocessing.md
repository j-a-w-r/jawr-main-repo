...
process could take some times, when the application contains a large
number of bundles.
...
try to use specific files, which are containing all the data about the
...
We have added the option to define explicitly the location the working
directory. By default the working directory is set in
...
This class is responsible of preprocessing the bundles, creating the
...
An Ant task and a maven plugin have been created to make it easy the
integration in the build process. For more information about the usage,
...
server startup, which you are used later in Jawr. These elements could
...
As the bundle processor simulate the server startup, the idea is to only
initialize the Jawr component. So with the **springConfigFiles**
parameter, we can choose exactly which beans must be instanciated, in
order to initialize the Jawr component.

For Spring MCV, Jawr provides the JawrSpringController which generates
...
configuration. So in that case, you must pass as argument to the bundle
processor the spring configuration file containing the
JawrSpringController declaration.

If your controller use dynamic properties, you can define these dynamic
...
The bundle processor generates also a configuration file, containing the
...
As explained above, the bundle preprocessing use the predefined mapping
...
resource file will never been taken in account, unless you clean your
working directory and start again your application.
...
that the production bundles are the one which have been processed during
the bundling phase. This means that if you have modified a resource
file, since the latest bundling process, you will have to launch again
the bundling process to have this updated resource file in your
production bundle.
