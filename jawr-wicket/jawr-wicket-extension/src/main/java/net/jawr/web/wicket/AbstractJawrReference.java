...
public abstract class AbstractJawrReference extends WebMarkupContainer {
		...
		try {
			...
			if (null == src) {
				throw new IllegalStateException(
						"The src attribute is mandatory for this Jawr reference tag. ");
			}
			...
			if (RendererRequestUtils.refreshConfigIfNeeded(request,
					rsHandler.getConfig())) {
				rsHandler = (ResourceBundlesHandler) request.getSession()
						.getServletContext()
						.getAttribute(getResourceHandlerAttributeName());
			}
	 ...
	 *            the flag indicating if we must use random parameter in debug
	 *            mode
	 * @param tag
	 *            the tag
	...
	protected abstract BundleRenderer createRenderer(
			ResourceBundlesHandler rsHandler, Boolean useRandomParam,
			ComponentTag tag);
